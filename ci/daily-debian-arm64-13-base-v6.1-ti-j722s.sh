...
config=bb.org-debian-trixie-base-v6.1-ti-arm64-k3-j722s
filesize=8gb
rootfs="debian-arm64-13-base-v6.1-ti"
...
compress_snapshot_image () {
	...
	if [ -f ./sbom.tar.gz ] ; then
		echo "  sbom: https://raw.githubusercontent.com/beagleboard/distros/refs/heads/main/sbom-temp/${device}-${export_filename}-${filesize}.sbom.tar.gz" >> ${yml_file}
	fi
	...
	sudo -uvoodoo cp -v ./sbom.tar.gz /mnt/mirror/rcn-ee.us/rootfs/${rootfs}/${time}/${device}-${export_filename}-${filesize}.sbom.tar.gz || true
}
