...
function populate_boot {
	...
	echo ""
	sudo mount ${MMC}1 ./disk
	sudo mkimage -A arm -O linux -T kernel -C none -a 0x80008000 -e 0x80008000 -n "Linux" -d ${DIR}/deploy/vmlinuz-* ./disk/uImage
	sudo mkimage -A arm -O linux -T ramdisk -C none -a 0 -e 0 -n initramfs -d ${DIR}/deploy/initrd.img-* ./disk/uInitrd

	sudo mkimage -A arm -O linux -T script -C none -a 0 -e 0 -n "Ubuntu 10.04" -d ${DIR}/tools/boot.cmd ./disk/boot.scr
...
function populate_rootfs {
	...
	sudo tar xfp ${DIR}/deploy/armel-rootfs-* -C ./disk/
	sudo umount ./disk
}
